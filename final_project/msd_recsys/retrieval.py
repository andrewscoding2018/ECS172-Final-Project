...
def build_candidate_pool(
    ...
    """
    # Validate that both retrievers were called on the same user set.
    # If they disagree, the long-format DataFrames they produce can't be safely
    # outer-merged on user_idx (the indices would refer to different users).
    if als_ids.shape[0] != sid_ids.shape[0]:
        raise ValueError(
            f"Retriever output shapes disagree on user count: "
            f"als_ids has {als_ids.shape[0]} users but sid_ids has {sid_ids.shape[0]}. "
            f"Make sure Cell B (ALS) and Cell C (SID) run on the same subset of "
            f"valid_users — e.g., apply MAX_USERS / SAMPLE_N in CELL A so both "
            f"retrievers see the same list."
        )
    n = als_ids.shape[0]
    ...
    def _flatten(retr_ids, retr_sc, retr_ix_to_item, score_col):
        # n_local is derived from this retriever's array, not the outer n,
        # so the closure-captured n bug can't recur.
        n_local = retr_ids.shape[0]
        user_flat = np.repeat(np.arange(n_local, dtype=np.int32), retr_ids.shape[1])
